...
{
  char *M_program = (char *)malloc(sizeof(char) *(3+std::string(start).length() +M * (std::string(end).length()+std::string(base_name).length()+1)));
  strcpy(M_program, start);

  for (int i = 0; i < M; i++){
    strcat(M_program, base_name);
    sprintf(M_program + strlen(M_program), "%d", i);
    //strcat(M_program, (char*)M));
...
int main()
{
    ...
    //
    cl_kernel my_kernel = clCreateKernel(prog, "dot_prod", &err);
    OPENCL_ERR_CHECK(err);
